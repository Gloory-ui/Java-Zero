    ...
    <>
      ...
      <main id="main" className="mx-auto flex max-w-2xl flex-col gap-10 px-4 py-12 leading-relaxed">
        ...
        <Section title="Без входа в аккаунт">
          <p>
            Прогресс по этапам, код, опыт, достижения, квесты дня, оформление профиля и настройки хранятся только в
            твоём браузере (localStorage). На сервер они не отправляются. Очистишь данные сайта в браузере — прогресс
            пропадёт.
          </p>
        </Section>
        ...
        <Section title="С аккаунтом">
          ...
          <List
            items={[
              ...
              "прогресс по этапам: последний код, отметки о сдаче, число попыток, использованные подсказки, опыт за этап;",
              "достижения с датой получения, выполненные квесты дня, серию и счётчики для достижений (защиты, квизы, запуски);",
              "профиль: ник, «о себе», оформление (цвет, рамка, баннер, титул, витрина) и настройки — характер AI-ментора и звук;",
              "аватар и баннер, если ты загрузил свои картинки.",
            ]}
          />
          <p>
        ...
        <Section title="Публичный профиль и таблица лидеров">
          <p>
            Профиль закрыт, пока ты сам не включишь «Показывать профиль всем» в настройках. Тогда по ссылке /u/твой-ник
            его видит любой: имя, ник, аватар, баннер, «о себе», оформление, уровень, опыт, число сданных этапов, серию,
            достижения с датами и месяц регистрации. Открытый профиль попадает в таблицу лидеров: имя, ник, аватар,
            титул, уровень и опыт. Почту, код, ошибки и ответы на защите не видит никто.
          </p>
          <p>
            Загруженные аватар и баннер хранятся как открытые файлы: их может открыть любой, у кого есть прямая ссылка,
            даже если профиль закрыт. Не загружай то, что не готов показать.
          </p>
          <p>
            Редкость достижений («есть у 3 % студентов») считается по всем аккаунтам, но наружу уходят только общие
            числа, без имён.
          </p>
        </Section>
        ...
        <Section title="Как удалить данные">
          <List
            items={[
              "прогресс отдельного квеста — «Настройки» в профиле → «Прогресс по квестам» → «Сбросить», изменение уходит и в аккаунт;",
              "публичный профиль — выключи «Показывать профиль всем» в настройках, профиль и строка в таблице лидеров сразу пропадут;",
